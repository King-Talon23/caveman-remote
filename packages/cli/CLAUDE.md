...
- Claude Code's Remote Control check is separate from the first-party escape hatch above and `_CLAUDE_CODE_ASSUME_FIRST_PARTY_BASE_URL` does NOT satisfy it: the base URL must really be `api.anthropic.com` (#947). Remote Control has THREE doors, per the 2.1.247 binary: `r.command("remote-control",{hidden:!0}).alias("rc")` plus the `--remote-control [name]` flag. The subcommand is hidden, so `claude --help` does not list it — do not conclude from the help output that it does not exist. The original `args.includes("remote-control")` caught the subcommand and missed both `rc` and the flag. Subcommand and alias match in FIRST POSITION only (`claude -p rc` is a prompt; unrouting it would drop compression and metering silently), flags match anywhere, and `--remote-control-session-name-prefix` never matches. Launching direct is only half the fix, and the reason is the opposite of the intuitive one. Measured on 2.1.247: a settings-file `env` entry fills a variable the environment does NOT carry and loses to an inherited one, so the direct launch's own strip of the gateway URLs is what hands the persistent `caveman enable claude` route an empty slot. `claudeRemoteControlEscape` lifts it by passing `--settings` — command-line settings outrank every settings file, and `env` merges per key, so the user's other entries survive — and only for a route the install journal says Caveman owns; a user's own pinned base URL is reported, never rewritten. Nothing on disk changes, so concurrent sessions keep routing. The lift covers ONE of three gates: read out of the 2.1.247 binary, Remote Control also demands first-party auth (bedrock/vertex/foundry/mantle/enterprise-gateway logins each block it) and a session that is not API-key-authed (`ANTHROPIC_API_KEY`/`ANTHROPIC_AUTH_TOKEN`/`apiKeyHelper` block it) — neither is ours to fix, so do not report the lift as "Remote Control works". The base-URL gate compares `new URL(v).host` against `api.anthropic.com` rather than testing definedness, which is why writing the host explicitly passes and why the leftover `_CLAUDE_CODE_ASSUME_FIRST_PARTY_BASE_URL=1` is inert rather than fatal.
